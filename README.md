...
4. 在 skill 之间交接：`/clarify` 产出清晰目标并持久化确认后的共享语言，`/plan` 产出可执行计划并记录已批准的 durable decision，`/execute` 完成改动，`/verify` 证明结论，`/reflect` 只提升可复用学习。
...
产出：可复用经验、未来触发条件、必要的补充 glossary 或 durable decision、更新的 artifact，或明确说明无需沉淀。
