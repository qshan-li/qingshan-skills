...
面向 AI 编程 agent 的轻量软件开发方法论。把 gstack、Superpowers、GSD、Matt Pocock's skills 的精华提炼成 6 个 skill，围绕八条主线展开：**保留工程控制权、外科手术式改动、按风险分级流程、共享领域语言、紧反馈环、符合语言生态的类型安全、新鲜上下文、先验证再下结论**。
...
qingshan-skills 与具体运行时无关，本质是让 agent 运行时读到根 skill 和六个 workflow skill。提供五种安装方式，按需选择。
...
脚本会验证仓库完整性，然后把根 skill 和 workflow skill 链接到 Claude Code、Codex 和通用 agent 目录。加 `--force` 可替换已有链接并生成带时间戳的备份：
...
脚本会把 `qingshan-skills`、`clarify`、`plan`、`execute`、`investigate`、`verify`、`reflect` 链接到 Claude Code、Codex 和通用 agent 的全局 skill 目录。加 `--force` 可替换已有链接：
...
其他运行时与详细选项见 [`docs/installation.md`](docs/installation.md)。
