...
qingshan-skills 与具体运行时无关，本质是让 agent 运行时读到根 skill 和六个 workflow skill。提供三种安装方式，按需选择。
...
适用于 Claude Code 与 Codex，用仓库根目录的同步脚本建立全局链接：

```bash
bash scripts/sync-global-skills.sh
```

脚本会把 `qingshan-skills`、`clarify`、`plan`、`execute`、`investigate`、`verify`、`reflect` 链接到 Claude Code 与 Codex 的全局 skill 目录。移动仓库后重新运行即可；若目标位置已存在且冲突，加 `--force` 可先把旧目标移到备份再链接。
