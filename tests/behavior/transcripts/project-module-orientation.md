...
```text
User: /clarify 阅读项目的 payments 模块，给我结构化地图和 uncertainties。
...
Agent: Runs an Uncertainty Pass. Evidence stays anchored to the map; open facts
cover webhook retry ownership; no open product decision exists because this is
orientation-only; blind-spot hypotheses cover provider idempotency and hidden
status mapping; provider behavior not represented in the repository is labeled
residual uncertainty. It lists the smallest read-only probes and the next route:
continue /clarify for webhook details or move to /plan only after a specific
change is requested.
...
- [project-module-orientation-unknowns-next-route] The transcript separates evidence, open facts, open decisions, blind-spot hypotheses, residual uncertainty, discovery probes, and next route.
