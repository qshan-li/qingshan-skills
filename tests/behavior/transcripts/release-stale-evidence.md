...
the PR only after the checklist status is ready, every Taste decision is
approved, and no User Challenge decision remains; otherwise it reports blocked
instead of acting.
```
...
- [release-stale-evidence-action-boundary] The transcript performs or hands off PR creation only after readiness is proven, Taste is approved, and no User Challenge remains.
