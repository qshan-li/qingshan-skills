...
action only after readiness is proven, every Taste decision is approved, and no
User Challenge decision remains.
...
- [release-stale-evidence-action-boundary] `/verify` performs or hands off release action only after readiness is proven, Taste is approved, and no User Challenge remains.
